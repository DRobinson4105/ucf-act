...
def render_overlay():
    ...
    base = cv2.addWeighted(config["img"], 0.5, color_mask, 0.5, 0)
...
def on_mouse(event, x, y, flags, param):
    if event == cv2.EVENT_MOUSEWHEEL:
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            # cv2.circle(config["label"], (xi, yi), 1, (255, 255, 255), -1)
            # cv2.circle(config["label"], (xi, yi), 0, (0, 0, 0), -1)
            config["curr_polygon"].append((xi, yi))
            render_overlay()
...
def main(args):
    ...
    print("s: save")
    print("c: segment current polygon")
    ...
    while True:
        ...
        if key == ord('s'):
            ...
            cv2.imwrite(save_label, config["label"])
            break
