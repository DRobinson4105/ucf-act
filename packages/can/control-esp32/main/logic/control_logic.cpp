...
/**
 * @brief Zero throttle and reset motor dedup trackers to safe defaults.
 *
 * Called when leaving ACTIVE or entering any non-driving state (retreat,
 * faulted cause, etc.).  Sets throttle to zero and marks both motor
...
{
	r->throttle_level = 0;
	r->new_last_steering = MOTOR_DEDUP_RESET_STEERING;
	r->new_last_braking = MOTOR_DEDUP_RESET_BRAKING;
}

static int32_t clamp_delta_toward_target(int32_t current, int32_t target, int32_t max_delta)
{
	if (max_delta <= 0)
		return target;
                                           ...
                                           const control_inputs_t *inputs)
{
	...
	case NODE_STATE_ACTIVE:
	{
		...
		// Braking PT targets are bounded per update so large planner jumps do
		// not turn into a single abrupt motor move. On ACTIVE entry, anchor the
		// first bounded step to the motor's measured current position.
		int32_t braking_reference = (inputs->last_braking_sent == MOTOR_DEDUP_RESET_BRAKING)
			                            ? inputs->current_braking_position
			                            : inputs->last_braking_sent;
		clamped_braking = clamp_delta_toward_target(braking_reference, clamped_braking, inputs->braking_step_limit);
		...
		// Motor dedup (uses clamped values already set in r.steering/braking_position)
