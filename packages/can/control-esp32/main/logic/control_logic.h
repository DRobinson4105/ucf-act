...
typedef struct
{
	...
	// Motor position error (computed by caller from motor feedback)
	...
	int32_t current_braking_position; // current brake motor position (used to seed PT slew on ACTIVE entry)
	int32_t braking_step_limit;       // max braking PT target delta per control step (<=0 disables slew clamp)
...
typedef struct
{
	...
	// Motor outputs (valid when state is ACTIVE and no override triggered)
