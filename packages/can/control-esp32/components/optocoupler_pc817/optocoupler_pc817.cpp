...
[[maybe_unused]] const char *TAG = "OPTOCOUPLER_PC817";

bool s_initialized = false;
optocoupler_pc817_config_t s_config = {};
fr_state_t s_state_debounced = FR_STATE_NEUTRAL;
fr_state_t s_state_pending = FR_STATE_NEUTRAL;
uint32_t s_change_time_ms = 0;
bool s_debouncing = false;

/**
 * @brief Decode F/R state from dual PC817 optocoupler GPIO inputs.
 *
 ...
 *
 * @return Decoded F/R gear state
 */
fr_state_t read_state_raw_internal(void)
{
	...
	if (forward_active && !reverse_active)
		return FR_STATE_FORWARD;
	if (forward_active && reverse_active)
		return FR_STATE_REVERSE;
	if (!forward_active && !reverse_active)
...
void optocoupler_pc817_update(uint32_t now_ms)
{
	...
	if (current_raw != s_state_debounced)
	{
		if (!s_debouncing)
		{
			...
			ESP_LOGI(TAG, "F/R change detected, starting debounce: %d -> %d", s_state_debounced, current_raw);
#endif
		}
