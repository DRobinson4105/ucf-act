...
[[maybe_unused]] const char *TAG = "ULTRASONIC";
...
constexpr UBaseType_t RX_TASK_PRIO = 5; // Below CAN RX (7) and safety (6)
...
portMUX_TYPE s_ultrasonic_lock = portMUX_INITIALIZER_UNLOCKED;
...
ultrasonic_a02yyuw_config_t s_config = {};
...
/**
 * @brief Reset the UART stream parser to its initial state.
 *
 * Clears the frame accumulation buffer and byte index so the parser
 * can resynchronize to the next 0xFF start byte.  Called during init
 * and deinit.
 */
void ultrasonic_a02yyuw_reset_parser_state(void)
{
	s_parser_idx = 0;
	for (int i = 0; i < 4; ++i)
		s_parser_frame[i] = 0;
 ...
 * @param distance_mm  [out] Parsed distance in millimeters (updated only on success)
 * @return true if at least one valid frame was parsed, false otherwise
 */
bool ultrasonic_a02yyuw_parse_stream(const uint8_t *data, int len, uint16_t *distance_mm)
{
	bool found = false;
		...
		if (s_parser_idx == 4)
		{
			...
			if (sum == s_parser_frame[3])
			{
				...
				else
				{
					...
					{
						ESP_LOGW(TAG, "Out-of-range sample dropped: %u mm (valid %u-%u)", parsed_mm, DISTANCE_MIN_MM,
						         DISTANCE_MAX_MM);
					}
#endif
				}
...
/**
 * @brief Store a validated distance reading under the module spinlock.
 *
 * Updates the shared distance, last-update tick, and has-measurement
 * flag atomically so the public API functions can read consistent
 * snapshots from any task context.
 *
 * @param distance_mm  Validated distance in millimeters
 */
void ultrasonic_a02yyuw_update_distance(uint16_t distance_mm)
{
...
/**
 * @brief FreeRTOS background task that continuously reads UART sensor data.
 *
 * Runs in a loop reading raw bytes from the A02YYUW UART port, feeding
 * them to the stream parser, and storing valid distance readings via
 * ultrasonic_a02yyuw_update_distance().  Registered with the task
 * watchdog to detect stuck reads.  Exits cleanly when s_stop_requested
 * is set by ultrasonic_a02yyuw_deinit().
 *
 * @param arg  Pointer to the internal ultrasonic_a02yyuw_config_t copy
 */
void ultrasonic_a02yyuw_uart_rx_task(void *arg)
{
	...
	ESP_LOGI(TAG, "UART RX task started (UART%d, TX=GPIO%d, RX=GPIO%d, %d baud)", config->uart_num, config->tx_gpio,
	         config->rx_gpio, config->baud_rate);
#endif

	// Register this task with the task watchdog timer so a stuck UART read
	// (e.g. driver failure) is detected rather than silently hanging.
	...
	while (!s_stop_requested)
	{
		...
		if (read_len > 0)
		{
			...
			if (read_len <= 8)
			{
				ESP_LOGI(TAG, "RX %d bytes: %02X %02X %02X %02X %02X %02X %02X %02X", read_len,
				         read_len > 0 ? buffer[0] : 0, read_len > 1 ? buffer[1] : 0, read_len > 2 ? buffer[2] : 0,
				         read_len > 3 ? buffer[3] : 0, read_len > 4 ? buffer[4] : 0, read_len > 5 ? buffer[5] : 0,
				         read_len > 6 ? buffer[6] : 0, read_len > 7 ? buffer[7] : 0);
			}
			else
			{
				ESP_LOGI(TAG, "RX %d bytes (showing first 8): %02X %02X %02X %02X %02X %02X %02X %02X ...", read_len,
...
esp_err_t ultrasonic_a02yyuw_init(const ultrasonic_a02yyuw_config_t *config)
{
	...
	BaseType_t task_ok = xTaskCreate(ultrasonic_a02yyuw_uart_rx_task, "ultrasonic_a02yyuw_rx", RX_TASK_STACK,
	                                 (void *)&s_config, RX_TASK_PRIO, &s_rx_task);
	if (task_ok != pdPASS)
	{
		uart_driver_delete(config->uart_num);
		s_uart_installed = false;
...
{
	uint16_t distance_mm = 0;
	if (!ultrasonic_a02yyuw_get_distance_mm(&distance_mm))
		return false;
