...
static void test_planner_and_control_faults(void)
{
	...
	assert(d.estop_active == true);
	assert(d.fault_code ==
	       (NODE_FAULT_ESTOP_PLANNER | NODE_FAULT_ESTOP_PLANNER_TIMEOUT | NODE_FAULT_ESTOP_CONTROL_TIMEOUT));
}
...
static void test_multiple_faults_all_bits(void)
{
	...
	safety_decision_t d = safety_evaluate(&in);
	assert(d.estop_active == true);
	assert(d.fault_code == (NODE_FAULT_ESTOP_BUTTON | NODE_FAULT_ESTOP_REMOTE | NODE_FAULT_ESTOP_ULTRASONIC |
	                        NODE_FAULT_ESTOP_PLANNER));
	assert(d.relay_enable == false);
}
...
// 24. NULL input -> fail-safe: estop_active=true, relay_enable=false
static void test_safety_evaluate_null_input(void)
{
...
int main(void)
{
	...
	// NULL input guard (1)
	printf("\n--- NULL input guard ---\n");
	TEST(test_safety_evaluate_null_input);
