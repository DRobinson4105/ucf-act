/**
 * @file test_stepper_protocol.cpp
...
// ============================================================================
// Safety configuration (IC) — hot-start protection
// ============================================================================

static void test_build_ic_auto_enable_off(void)
{
	uint8_t data[8];
	uint8_t dl = stepper_uim2852_build_ic_set(data, STEPPER_UIM2852_IC_AUTO_ENABLE, 0);
	assert(dl == 3);
	assert(data[0] == STEPPER_UIM2852_IC_AUTO_ENABLE); // index 0
	assert(data[1] == 0);                              // value low byte
	assert(data[2] == 0);                              // value high byte

	uint8_t idx;
	int32_t val;
	assert(stepper_uim2852_parse_param_response(data, dl, &idx, &val));
	assert(idx == STEPPER_UIM2852_IC_AUTO_ENABLE);
	assert(val == 0);
}
...
static void test_build_ic_software_limits_on(void)
{
	...
	assert(dl == 3);
	assert(data[0] == STEPPER_UIM2852_IC_SOFTWARE_LIMITS); // index 7
	assert(data[1] == 1);
	assert(data[2] == 0);
}

static void test_ic_parameter_indices(void)
{
	// Verify IC parameter indices match spec
	assert(STEPPER_UIM2852_IC_AUTO_ENABLE == 0);
	assert(STEPPER_UIM2852_IC_DIR_POLARITY == 1);
	assert(STEPPER_UIM2852_IC_LOCKDOWN == 3);
...
// ============================================================================
// PT/PVT interpolated motion builders
// ============================================================================
...
{
	uint8_t data[8];
	uint8_t dl = stepper_uim2852_build_qp(data, -500);
	assert(dl == 4);
	// -500 = 0xFFFFFE0C
	assert(data[0] == 0x0C);
	assert(data[1] == 0xFE);
...
static void test_build_qv(void)
{
	uint8_t data[8];
	uint8_t dl = stepper_uim2852_build_qv(data, 3200);
	assert(dl == 4);
	assert(data[0] == 0x80);
	assert(data[1] == 0x0C);
}

static void test_build_qt(void)
{
	uint8_t data[8];
	uint8_t dl = stepper_uim2852_build_qt(data, 1000);
	assert(dl == 4);
	// 1000 = 0x3E8
	assert(data[0] == 0xE8);
	assert(data[1] == 0x03);
...
int main(void)
{
	...
	// PT/PVT interpolated motion
	TEST(test_build_pv_pt_start);
	TEST(test_build_pv_pvt_stop);
	TEST(test_build_pt_positive);
	TEST(test_build_pt_negative);
	TEST(test_build_qf);
	TEST(test_build_qp);
	TEST(test_build_qv);
	TEST(test_build_qt);
